...
const mainSavePath = `FiveM-images`;
try {
    if (!fs.existsSync(mainSavePath)) {
        fs.mkdirSync(mainSavePath, { recursive: true });
    }
 /**
  * Event handler for taking a screenshot, processing it, and saving it to a specified path.
  *
  * @param {string} filename - The name of the file to save the screenshot as.
  * @param {string} type - The type or category of the screenshot, used to determine the save path.
